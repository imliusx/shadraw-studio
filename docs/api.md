...
```json
{
  "data": {
    "user": {
      ...
      "email": "alice@example.com",
      "displayName": "Alice",
      "avatarUrl": "/api/v1/auth/avatar/12",
      "role": "user",
      "mustChangePassword": false,
      "createdAt": "2026-05-25T11:08:00Z"
    }
...
- 200 响应：`{ "data": { "user": UserDTO }, "error": null }`
- 422：昵称为空或超过 32 个字符。
...
- 限制：JPG / PNG，最大 2MB。上传成功后旧头像 blob 会被删除。
- 200 响应：`{ "data": { "user": UserDTO }, "error": null }`
- 422：缺少文件、文件过大或类型不是 JPG / PNG。
...
- 鉴权：无
- 200 响应：`image/png` 或 `image/jpeg`
- 404：用户不存在、未设置头像或 blob 缺失。
...
- 200 响应：`{ "data": { "users": [AdminUserDTO] }, "error": null, "meta": ... }`
- `AdminUserDTO` 在公共用户字段基础上增加 admin 专用状态字段 `disabled`：
...
- 鉴权：Bearer + admin
- 请求体：`{ "disabled": true }` 或 `{ "role": "admin" }`
- 200 响应：`{ "data": { "user": AdminUserDTO }, "error": null }`
- 404：用户不存在。

## POST /admin/users/:id/reset-password

管理员为用户生成临时密码，并要求用户下次登录后修改密码。

- 鉴权：Bearer + admin
- 200 响应：`{ "data": { "tempPassword": "..." }, "error": null }`
- 404：用户不存在。
